...
def process_chunk_for_process(tid, chunk, idx_in_chunk, chunksize, create_gt_database_of_single_scene, file_prex):
    ...
    for i, info_with_idx in enumerate(zip(chunk, idx_in_chunk)):
        ...
        if i >= break_chunk:
            break
        chunk_list.append(create_gt_database_of_single_scene(info_with_idx, tid, chunk_id))
        if (i+1) % 100 == 0:
            print('process_%02d: %04d/%06d' % (tid, i+1, chunksize))
        if (i+1) % interval == 0:
            file = file_prex / ("p_%02d_%04d.pkl" %(tid, int(i/interval) + 1 ))
            with open(file, 'wb') as f:
                pickle.dump(chunk_list, f)
                print("%s is saved" %(str(file)) )
            chunk_list = []
            gc.collect()
        elif (i+1) == chunksize:
            file = file_prex / ("p_%02d_%04d.pkl" %(tid, int(i/interval) + 1))
            with open(file, 'wb') as f:
                ...
                print("%s is saved" %(str(file)))
    gc.collect()
    print("process_%02d finished" % (tid))
class WaymoDataset(DatasetTemplate):
    ...
    def include_waymo_data(self, mode):
        ...
        for k in range(len(self.sample_sequence_list)):
            ...
            with open(info_path, 'rb') as f:
                # print("loading infos from %s" %(info_path))
                infos = pickle.load(f)
        ...
        if self.dataset_cfg.SAMPLED_INTERVAL[mode] > 1:
            print('force to excute 20% sampling')
            sampled_waymo_infos = []
    ...
    def create_gt_database_of_single_scene(self, info_with_idx,  pid=None, chunkid=None, database_save_path=None, use_sequence_data=False, used_classes=None,
        ...
        for i in range(num_obj):
            filename = '%s_%04d_%s_%d.bin' % (sequence_name, sample_idx, names[i], i)
            filepath = database_save_path / filename
            if pid is not None and chunkid is not None:
                process_child_path = database_save_path / ('process_%02d' % pid)
                if not process_child_path.exists():
                    print("%s is maked" %(str(process_child_path)))
                    process_child_path.mkdir(parents=False, exist_ok=False)
                
                chunk_child_path = process_child_path / ('chunk_%04d' % chunkid)
                if not chunk_child_path.exists():
                    chunk_child_path.mkdir(parents=False, exist_ok=False)
                    print("%s is maked" %(str(chunk_child_path)))
                    
                filepath = chunk_child_path / filename
            ...
            if (used_classes is None) or names[i] in used_classes:
                if not filepath.exists():
                    ...
                    gt_points = np.fromfile(str(filepath), dtype=np.float32).reshape(-1, self.dataset_cfg['DATA_AUGMENTOR']['AUG_CONFIG_LIST'][0]['NUM_POINT_FEATURES'])
                assert gt_points.dtype == np.float32
    ...
    def create_groundtruth_database_parallel(self, info_path, save_path, used_classes=None, split='train', sampled_interval=10,
        ...
        print('time cost for reading info:', end-start, "s")
        print(f'Number workers: {num_workers}')
        # global create_gt_database_of_single_scene
        ...
        if backend == 'process':
            ...
            processes = []
            start_index =0 
            info_idx = np.arange(len(infos))
            chunksize = int(len(infos)/num_workers)
            ...
            for i in range(num_workers):
                ...
                info_idx = info_idx[start_index+chunksize:]
                gc.collect()
            for i in range(num_workers):
                processes[i].join()
            gc.collect()
            ...
            for _p in os.listdir(chunk_gt_database_prefix):
                ...
                for i, cur_db_infos in enumerate(chunk):
                    print("processing %d/%d in %s" %(i+1, 1000, _p))
            ...
            all_db_infos_chunk_list = [[] for i in range(num_workers)]
            info_idx = np.arange(len(infos))
            chunksize = int(len(infos)/num_workers)
            print("chunksize:%d" % (chunksize))
            def process_chunk(tid, chunk, chunk_idx):
                for i, info_with_idx in enumerate(zip(chunk, chunk_idx)):
                    if i < 16000:
                        print('thread_:%d: %d/%d' % (tid, i, chunksize))
                        all_db_infos_chunk_list[tid].append(create_gt_database_of_single_scene(info_with_idx))
            ...
            for i in range(num_workers):
                # print()
                t = Thread(target=process_chunk, args=(i, infos[start_index:start_index+chunksize], info_idx[start_index:start_index+chunksize]))
            ...
            for i in range(num_workers):
                threads[i].join()
            all_db_infos = {}
            for cur_db_infos_list in all_db_infos_chunk_list:
                for cur_db_infos in cur_db_infos_list:
                    for key, val in cur_db_infos.items():
                        if key not in all_db_infos:
                            all_db_infos[key] = val
                        else:
...
def create_waymo_gt_database(
    ...
    workers=min(16, multiprocessing.cpu_count()), use_parallel=False, crop_gt_with_tail=False, backend='thread'):
    ...
    if use_parallel:
        dataset.create_groundtruth_database_parallel(
            info_path=train_filename, save_path=save_path, split='train', sampled_interval=1,
            used_classes=['Vehicle', 'Pedestrian', 'Cyclist'], processed_data_tag=processed_data_tag,
            num_workers=workers, crop_gt_with_tail=crop_gt_with_tail, backend=backend
        )
    else:
        dataset.create_groundtruth_database(
            info_path=train_filename, save_path=save_path, split='train', sampled_interval=1,
...
def generate_db_database_path(info_path, save_path, data_path, cfg, db_data_save_path, new_info_path):
    with open(str(save_path/info_path), 'rb') as f:
        db_info = pickle.load(f)
    new_db_infos = {}
    stacked_gt_points = []
    start_cnt = 0
    db_data_save_path = save_path / db_data_save_path
    new_info_path = save_path / new_info_path
    print("info_path:%s" %(str(save_path/info_path)))
    print('db_data_save_path:%s' %(str(db_data_save_path)))
    print("new_info_path:%s" %(str(new_info_path))  )
    # print('db_gt_datebase:%s' %(db_gt_datebase))
    for k, v in db_info.items():
        for i, _info in enumerate(v):
            print("%d/%d" %(i+1, v.__len__()))
            info_path = data_path/_info['path'] 
            obj_points = np.fromfile(str(info_path), dtype=np.float32).reshape(
                    -1, cfg['DATA_AUGMENTOR']['AUG_CONFIG_LIST'][0]['NUM_POINT_FEATURES'])
            # print(obj_points.shape)
            stacked_gt_points.append(obj_points)
            # start_cnt
            _info['global_data_offset'] = [start_cnt, start_cnt + obj_points.shape[0]]
            start_cnt = start_cnt + obj_points.shape[0]
            if k not in new_db_infos:
                new_db_infos[k] = [_info]
            else:
                new_db_infos[k].append(_info)
...
def process_db_info(k_v_pair, data_path, cfg, start_cnt, tmp_save_path):
    k, v = k_v_pair
    task_len = v.__len__()
    new_db_infos = {}
    stacked_gt_points = []
    path_list = []
    local_start_cnt = start_cnt
    chunk_list = []
    interval = 10000
    print(tmp_save_path)
    print("process for %s" % k)
    for i, _info in enumerate(v):
        # if (i + 1) > 300:
        #     break
        if (i +1) % 100 == 0:
            print("%d|%d in %s" %(i+1, task_len, k))
        info_path = os.path.join(data_path, _info['path'])
        obj_points = np.fromfile(info_path, dtype=np.float32).reshape(
            -1, cfg['DATA_AUGMENTOR']['AUG_CONFIG_LIST'][0]['NUM_POINT_FEATURES'])
        
        stacked_gt_points.append(obj_points)
        assert obj_points.shape[0] == _info['num_points_in_gt'], "occur in %s" % info_path
        _info['global_data_offset'] = [local_start_cnt, local_start_cnt + obj_points.shape[0]]
        local_start_cnt += obj_points.shape[0]
        if k not in new_db_infos:
            new_db_infos[k] = [_info]
        else:
            new_db_infos[k].append(_info)
        if (i+1) % interval == 0:
            tmp_save_file = tmp_save_path / ('%s_%04d_points_chunk.npy' %(k, int(i/interval) + 1))
            print('%s is saved' % str(tmp_save_file))
            stacked_gt_points = np.concatenate(stacked_gt_points, axis=0)
            np.save(tmp_save_file, stacked_gt_points)
            stacked_gt_points = [] # clear the stacked points
            path_list.append(str(tmp_save_file)) # save path into ordered list
        if (i+1) == task_len:
            tmp_save_file = tmp_save_path / ('%s_%04d_points_chunk.npy' %(k, int(i/interval) + 1))
            stacked_gt_points = np.concatenate(stacked_gt_points, axis=0)
            np.save(tmp_save_file, stacked_gt_points)
            ...
            print('%s is saved' % str(tmp_save_file))
        gc.collect()
    path_list_file = tmp_save_path / ('%s_paths.pkl' %(k))
    with open(path_list_file, 'wb') as f:
        pickle.dump(path_list, f)
        print("file dumped in %s" %(str(path_list_file)) )
    print("process  named process_db_info for %s finished" % k)
    
    return new_db_infos, path_list, local_start_cnt

def update_global_offset(k_v_pair, start_cnt_kv):
    k, v = k_v_pair
    start_k, start_cnt = start_cnt_kv
    task_len = v.__len__()
    new_db_infos = {}
    assert k == start_k
    print("update_global_offset process for %s" % k)
    for i, _info in enumerate(v):
        if (i +1) % 100 == 0:
            print("%d|%d in %s" %(i+1, task_len, k))
        start, end = _info['global_data_offset']
        _info['global_data_offset'] = [start+start_cnt, end+start_cnt]
...
if __name__ == '__main__':
    ...
    ROOT_DIR = (Path("/opt/data/private/detection3d/")).resolve()
    ...
    if args.func == 'create_waymo_infos':
        ...
        print('current multiprocessing.start_method is:', multiprocessing.get_start_method())
    ...
    elif args.func == 'generate_db_database_path':
        try:
            yaml_config = yaml.safe_load(open(args.cfg_file), Loader=yaml.FullLoader)
        except:
            yaml_config = yaml.safe_load(open(args.cfg_file))
